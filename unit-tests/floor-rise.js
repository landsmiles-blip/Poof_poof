...
import { spawnFruit, hardDrop, isGameOver, raiseFloor, setDragTarget } from '../js/physics.js';
import {
...
// --- 3. THE losing rule: no move left, and nothing else -------------------
// Three designs have ended a run in this game. 17 killed on any full column
// (a loss with fifty free cells on the board). 20 gave a capped column one
// cadence of grace, then killed it. 21 is the rule the game should always
// have had: you lose when there is nowhere to put the next fruit. These tests
// hold that line, and section 4 keeps the run ending anyway.
{
  // One column capped, five wide open. The rise proceeds; the capped column's
  // top fruit is crushed off and it stays exactly as tall as it was.
  const state = createInitialState();
  startRun(state, {});
  const rows = state.grid.length;
  // Alternating tiers: a column of identical fruit merges with itself and
  // evaporates, which would silently un-fill the board this test is about.
  for (let r = 0; r < rows; r++) state.grid[r][0] = r % 2 ? 5 : 6;
  state.stackHeight[0] = rows;
  state.events.length = 0;

  assert.equal(raiseFloor(state).toppedOut, false,
    'a full column must NOT end the run -- there are still moves to play');
  assert.equal(state.stackHeight[0], rows, 'the capped column stays full: one crushed off the top, one added at the bottom');
  assert.ok(state.events.some((e) => e.type === 'crushed' && e.col === 0),
    'and the crush is announced, so it can be shown rather than just happening');
  for (let c = 1; c < COLS; c++) {
    assert.equal(state.stackHeight[c], 1, `column ${c} had room, so it grew`);
  }

  // Bottom-packing must survive a rise that crushes, or every later merge is wrong.
  for (let c = 0; c < COLS; c++) {
    let count = 0, seenFruit = false;
    for (let r = 0; r < rows; r++) {
      if (state.grid[r][c] !== null) { count += 1; seenFruit = true; }
      else assert.ok(!seenFruit, `column ${c} stays bottom-packed after a crush (hole at row ${r})`);
    }
    assert.equal(state.stackHeight[c], count, `column ${c}: stackHeight matches the grid after a crush`);
  }
}
{
  // Even a board with every column capped does not end IN raiseFloor. It ends
  // where the player would notice it: there is nowhere to put the next fruit.
  // One rule, checked in one place.
  const state = createInitialState();
  startRun(state, {});
  const rows = state.grid.length;
  // Alternating on BOTH axes: a uniform column merges with itself and would
  // quietly un-fill the board this test is about.
  for (let c = 0; c < COLS; c++) {
    for (let r = 0; r < rows; r++) state.grid[r][c] = (r + c) % 2 ? 5 : 6;
    state.stackHeight[c] = rows;
  }
  assert.equal(raiseFloor(state).toppedOut, false, 'a rise never ends the run on its own any more');
  assert.equal(spawnFruit(state).blocked, true, 'the run ends because there is no move to play');
}
{
  // A crushed column is not a free-deletion machine. Crushing costs the
  // player a fruit they placed and gains them a tier-0 or tier-1 at the
  // bottom, so a capped column is a liability, not a tactic. Measured over
  // 100 runs each: deliberately capping a column scored 48,666 against
  // 87,141 for normal play. Asserted here structurally: the column does not
  // get shorter, so no space is ever won by letting it fill.
  const state = createInitialState();
  startRun(state, {});
  const rows = state.grid.length;
  for (let r = 0; r < rows; r++) state.grid[r][0] = r % 2 ? 5 : 6;
  state.stackHeight[0] = rows;
  const before = state.stackHeight[0];
  raiseFloor(state);
  assert.equal(state.stackHeight[0], before, 'crushing never frees a cell -- it is a cost, not a reward');
}
